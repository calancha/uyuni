...
Then(/^the SLE15SP3 base products should be added$/) do
  output = sshcmd('echo -e "admin\nadmin\n" | mgr-sync list channels', ignore_err: true)
  raise unless output[:stdout].include? '[I] SLE-Product-SLES15-SP3-Pool for x86_64 SUSE Linux Enterprise Server 15 SP3 x86_64 (ALPHA) [sle-product-sles15-sp3-pool-x86_64]'
  raise unless output[:stdout].include? '[I] SLE-Module-Basesystem15-SP3-Updates for x86_64 Basesystem Module 15 SP3 x86_64 [sle-module-basesystem15-sp3-updates-x86_64]'
  raise unless output[:stdout].include? '[ ] SLE-Module-Server-Applications15-SP3-Pool for x86_64 Server Applications Module 15 SP3 x86_64 [sle-module-server-applications15-sp3-pool-x86_64]'
end
